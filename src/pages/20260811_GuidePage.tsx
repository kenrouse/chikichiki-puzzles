...
const GUIDE_COPY = {
  ja: {
    ...
    sudoku: {
      title: '一意解を保ったまま、数字を抜く',
      intro: '最初に完成した 9 × 9 の盤面を作り、行、列、3 × 3 ブロックの制約を満たすよう数字を並べ替えます。その後、マスを 1 つずつ空欄にします。数字を抜くたびに解を最大 2 個まで探索し、解が 1 個のときだけ変更を採用します。',
      flowLabel: '数独生成の流れ',
      flow: ['完成盤', '数字を抜く', '解を数える', '一意解を採用'],
      difficultyTitle: '難易度は、手掛かりの数だけでは決まりません',
      difficulty: '空欄が多くても、裸のシングルだけで解ける問題は難しくありません。現代版では、確定できるマスの数、探索で分岐した回数、探索ノード数を組み合わせて難易度を評価します。さらに難しくする場合は、ロック候補、ペア、X-Wing、XY-Wing、連鎖など、人間が使う解法を順番に適用し、最も高度な手筋で等級を決める方法が有効です。',
      ...
      rating: '現行版は全段階で解の個数を最大2個まで探索し、一意解だけを採用します。ratingは空欄数、裸／隠れシングル後の未解決数、バックトラック探索の分岐数とノード数から算出します。特にエキスパートは旧版に対応レベルがなく、複数候補から探索複雑度の高い問題を選ぶ2026年版独自の難易度です。',
      futureTitle: '発展案',
      future: 'Algorithm X / Dancing Links で一意解を高速判定し、回転対称な最小問題や Killer Sudoku の和制約へ拡張できます。',
    },
    mines: {
      title: '最初の 1 手が決まってから地雷を置く',
      intro: '盤面を開いた瞬間に、選んだマスと周囲 8 マスを安全領域として地雷を配置します。空白マスからは幅優先探索で周囲を開きます。右クリックまたはタッチ／ペンの長押しでは、旗、?、解除の順にマーカーを切り替えます。指やペンが一定距離動いた場合はドラッグとして扱い、開封と長押しマーカーをキャンセルします。共有 URL には seed と最初の 1 手を記録するため、受け取った側でも同じ初期展開とスコアを再現できます。',
  ...
  en: {
    ...
    sudoku: {
      title: 'Remove numbers while preserving a unique solution',
      intro: 'First, the generator creates a complete 9 × 9 board and rearranges rows, columns, and digits while preserving every Sudoku constraint. It then empties cells one by one. After each removal, it searches for up to two solutions and keeps the change only when exactly one solution remains.',
      ...
      difficulty: 'A puzzle with many empty cells can still be easy when every step is a naked single. The rebuilt version combines direct placements, unresolved cells after basic logic, search branches, and search nodes. A fuller human-style rating could also apply locked candidates, pairs, X-Wing, XY-Wing, and chains in order.',
      ...
      tableLabel: 'Current Sudoku difficulty generation',
      ...
      rating: 'Every current level counts solutions up to two and accepts only uniquely solvable puzzles. The rating combines empty cells, unresolved cells after naked and hidden singles, backtracking branches, and search nodes. Expert has no equivalent in the original version; it is a 2026 addition that selects the most search-intensive of six candidates.',
      futureTitle: 'Possible extensions',
      future: 'Algorithm X / Dancing Links could speed up uniqueness checks and support rotationally symmetric minimal puzzles or Killer Sudoku sum constraints.',
    },
    mines: {
      title: 'Place mines after the first move is known',
      intro: 'When the first cell is opened, the game places mines while reserving that cell and its eight neighbors as safe. A breadth-first search expands empty areas. Right-click or touch/pen hold cycles Flag, ?, and Clear. Moving beyond a threshold turns the gesture into a drag and cancels both opening and long-press marking. Shared URLs include the seed and first move, reproducing the same layout and opening.',
