...
- **おーとまちっく数独**: 唯一解を保証した問題生成、候補メモ、Undo、ヒント、複雑度評価付き4段階の難易度
- **ちきちきまいんすいーぱ。**: 初手安全、旗、周囲の一括展開、連鎖スコア、3段階の盤面
- **四川省**: 2回以内に曲がる経路探索、必ず解ける配牌、mobility評価付き3段階の難易度

全ゲームでseed付きURLとQRコードによる盤面共有、開始カウントダウン、成績画面、4つのカラーテーマ、生成BGM、効果音、音量調整に対応しています。マインスイーパはseedと初手を共有し、地雷配置と初期展開を同じ状態で再現します。
