...
- **おーとまちっく数独**: 唯一解を保証した問題生成、左クリックの確定／メモ切替、右クリック候補メモ、確定値からメモへの置換、Undo、ヒント、任意の同数字ハイライト、複雑度評価付き4段階の難易度
